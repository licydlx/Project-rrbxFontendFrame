 var dateUnit = {
 	...
 	// 产品:平安e生保 团体版
 	// 逻辑:每月27号（含27号）前购买，次月1日零时生效；每月27号后购买，隔月1日零时生效；
 	// 条件:...
 	// 例子:
 	// 作者:ydlx
 	// 日期:2018-6-4
 	getEffectDate: function() {
 		...
 		var effectYear, effectMonth, effectDay;

 		if (curDay > 27) {
 			if (curMonth + 2 > 12) {
 				effectYear = curYear + 1;
 				effectMonth = curMonth - 10;
 				effectDay = 1;
 			} else {
 				effectYear = curYear;
 				effectMonth = curMonth + 2;
 				effectDay = 1;
 			};
 		} else {
 			if (curMonth + 1 > 12) {
 				effectYear = curYear + 1;
 				effectMonth = curMonth - 11;
 				effectDay = 1;

 			} else {
 				effectYear = curYear;
 				effectMonth = curMonth + 1;
 				effectDay = 1;
 			};
 		};
 		...
 		if (effectDay < 10) {
 			effectDay = "0" + effectDay
 		};

 		return effectYear + "-" + effectMonth + "-" + effectDay;
 	}
 }
