...
const serviceLogic = function(a) {
	...
	function holderArea(value) {
		trialObj.extraParams.holderResidentProvince = value.selectOneObj.value;
		trialObj.extraParams.holderResidentCity = value.selectTwoObj.value;
		trialObj.extraParams.holderResidentCounty = value.selectThreeObj.value;
		return true;
	};
	...
	$("#container").on("blur", "input[data-type='certiNo']", function(event) {
		...
		if (relaState && Object.is("holder_certiNo", certiNoId)) {
			...
			if (!curAge) {
				...
				if (!flag) {
					new dateModal(null, "stateIndform", "被保人年龄最小20周岁,最大44周岁").init().show();
					$("#holder_certiNo").val('').closest('.item').attr('data-state', '');
				}

				if (cardObj.sex == "men") {
					new dateModal(null, "stateIndform", "被保人只能为女性").init().show();
					$("#holder_certiNo").val('').closest('.item').attr('data-state', '');
				}
			}
